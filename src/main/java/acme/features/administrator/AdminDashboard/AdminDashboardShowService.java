...
public class AdminDashboardShowService implements AbstractShowService<Administrator, AdminDashboard> {
	...
	//	Examen ------------------------------------------------------------------------------------------
		...
		final int nChimpum = this.repository.findAllNewcal().size();
		final Double nArtifacts = Double.valueOf(this.repository.findArtifact(ArtifactType.COMPONENT).size());
		...
		final Collection<Newcal> chimpumList = this.repository.findAllNewcal();
		...
		for(final Newcal c: chimpumList) {
